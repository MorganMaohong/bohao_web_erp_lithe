...
const detailData = ref<InventoryInboundDetail | null>(null)

function createEmptyForm(): InventoryInboundForm {
  return {
    code: '',
    type: '',
    otherType: '',
...
const form = reactive<InventoryInboundForm>(createEmptyForm())
...
function syncForm(data?: Partial<InventoryInboundForm>) {
...
function ensureDetailListValid() {
  if (!form.detailList?.length) {
    message.error('入库明细不能为空')
    return false
  }

  for (const item of form.detailList) {
    if (!Number(item.quantity || 0)) {
      message.error(`【${item.name || '未命名物料'}】入库数量必须大于 0`)
      return false
    }
  }
...
async function openCreateModal() {
  ...
  const payload = await getInventoryInboundForm()
  syncForm(payload)
  normalizeWarehouseSelection()
  showEditModal.value = true
}

async function openEditModal(row: InventoryInboundRow) {
  activeEditId.value = row.uid || null
  const payload = await getInventoryInboundForm(row.uid || undefined)
  syncForm(payload)
  normalizeWarehouseSelection()
  showEditModal.value = true
}
...
async function submitForm(mode: 'save' | 'complete') {
  ...
  try {
    if (mode === 'save') {
      await saveInventoryInbound({ ...form, detailList: [...(form.detailList || [])] })
      message.success('入库单已保存')
    } else {
      await completeInventoryInbound({ ...form, detailList: [...(form.detailList || [])] })
...
async function handleCancel(row: InventoryInboundRow) {
  if (!row.uid) return
  await cancelInventoryInbound(row.uid)
  message.success('入库单已取消')
  loadTableData()
}

async function removeDetailRow(row: InventoryInboundItemDetail, index: number) {
  if (row.uid) {
    await deleteInventoryInboundDetail(row.uid)
  }
...
const columns = computed<DataTableColumns<InventoryInboundRow>>(() => [
  ...
  { title: '客户/来货方', key: 'customerName', width: 160, ellipsis: { tooltip: true } },
  ...
  { title: '入库时间', key: 'timeName', width: 180, ellipsis: { tooltip: true } },
      ...
      h('div', { class: 'inbound-item-cell' }, [
        row.image
          ? h(NImage, {
              ...
              class: 'inbound-item-image',
            })
          : h('div', { class: 'inbound-item-image inbound-item-image--empty' }, '无图'),
        h('div', { class: 'min-w-0' }, [
...
const detailViewColumns = computed<DataTableColumns<InventoryInboundItemDetail>>(() => [
  {
    title: '物料',
    key: 'name',
    minWidth: 240,
...
<template>
  <div class="flex h-full min-h-0 flex-col gap-3 p-3">
    <NCard
      size="small"
      :bordered="false"
      class="inbound-query-card"
    >
      <NForm
        ref="queryFormRef"
        class="inbound-query-form"
      >
        <div class="inbound-query-bar">
          <NFormItem class="inbound-query-item inbound-query-item--keyword">
            <NInput
          ...
          <NFormItem class="inbound-query-item">
            <NSelect
          ...
          <NFormItem class="inbound-query-item inbound-query-item--actions">
            <NFlex
              size="small"
              class="inbound-query-actions"
            >
              <NButton
                type="primary"
                attr-type="button"
    ...
    >
      <div class="mb-3 flex items-center justify-between">
        <div class="text-sm text-neutral-500 dark:text-neutral-400">其他入库记录</div>
      ...
      <div class="flex max-h-[78vh] min-h-0 flex-col gap-4 overflow-hidden">
        ...
        >
          <div class="grid grid-cols-1 gap-x-4 gap-y-1 md:grid-cols-3">
            ...
            <NFormItem label="客户/来货方">
              <NInput
                v-model:value="form.customerName"
                placeholder="请输入客户或来货方"
              />
            </NFormItem>
            <NFormItem label="经手人">
              <NInput
        ...
        >
          <div class="mb-3 flex items-center justify-between">
            <div>
              <div class="text-sm font-medium text-neutral-900 dark:text-neutral-100">入库明细</div>
      ...
      <div class="flex max-h-[72vh] min-h-0 flex-col gap-3">
        ...
        <div class="flex items-center justify-between gap-3">
          ...
          <div class="flex gap-2">
            <NButton @click="showItemPicker = false">取消</NButton>
            <NButton
              type="primary"
      ...
      >
        ...
        >
          ...
          >
            ...
            <NDescriptionsItem label="客户/来货方">{{ detailData.customerName || '-' }}</NDescriptionsItem>
...
<style scoped>
.inbound-query-form:deep(.n-form-item) {
  margin-bottom: 0;
}

.inbound-query-form:deep(.n-form-item-label) {
  display: none;
}

.inbound-query-card:deep(.n-card__content) {
  padding: 12px 16px;
}
...
.inbound-query-item--keyword {
  flex: 1.8 1 220px;
}

.inbound-query-item--actions {
  flex: 0 0 auto;
  min-width: auto;
}
...
.inbound-query-actions :deep(.n-button) {
  padding-left: 10px;
  padding-right: 10px;
}
...
:global(.dark) .inbound-item-image--empty {
